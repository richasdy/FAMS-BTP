...
public class AdapterScannedCode extends BaseAdapter {
    ...
    public void hapusData(int posisi) {
        for (int i = 0; i < dataCodeOrigin.size(); i++) {
            if (dataCodeOrigin.get(i).getCode().equals(dataCode.get(posisi).getCode())) {
                dataCodeOrigin.remove(i);
                break;
            }
        }
        dataCode.remove(posisi);
        notifyDataSetChanged();
    }

    public void filterbylokasi(String klasifikasi) {
        dataCode.clear();
        for (ScannedCode dataAsli : dataCodeOrigin) {
            if (!dataAsli.isNewData()) {
                if (dataAsli.getLokasi().getName().equals(klasifikasi)) {
                    dataCode.add(dataAsli);
                }
            }
        }
